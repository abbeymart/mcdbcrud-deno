...
export function shortString(str: string, maxLength = 20): string {
  return str.toString().length > maxLength
    ? str.toString().substr(0, maxLength) + "..."
    : str.toString();
}
...
export function strToBool(val: string | number = "n"): boolean {
  ...
  if (
    strVal === "true" || strVal === "t" || strVal === "yes" || strVal === "y"
  ) {
    return true;
  } else {
...
export async function userIpInfo(
...
export function currentUrlInfo(pathLoc: string) {
  ...
  if (pathLoc) {
    ...
    // get the last index
    lastIndex = parts.lastIndexOf("new") || parts.lastIndexOf("detail") ||
      parts.lastIndexOf("list");
    return {
      parts,
      lastIndex,
    };
