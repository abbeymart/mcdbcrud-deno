...
class LoadRecord<T extends ValueType> {
  ...
  async loadRecord<T>(): Promise<ResponseMessage<T>> {
    ...
    if (totalRecordCount > this.maxQueryLimit) {
      errors.maxQueryLimit =
        `${this.actionParams.length} records load-request, exceeded ${this.maxQueryLimit} limit. 
      ...
      try {
        ...
        if (res.rows.length > 0) {
          return getResMessage("success", {
            message:
              `${res.rows.length} of ${totalRecordCount} record(s) created successfully.`,
            value: {
