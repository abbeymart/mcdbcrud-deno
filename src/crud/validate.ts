...
export const isObjectType = (param: unknown): boolean => {
  "use strict";
  // Validate param is an object, {}
  return (typeof param === "object" && !Array.isArray(param));
};

export const isArrayType = (param: Array<unknown>): boolean => {
  "use strict";
  // Validate param is an object, []
...
export const isNull = (infoItem: unknown): boolean => {
  "use strict";
...
export const isEmail = (param: string): boolean => {
  const testPattern =
    /^[\da-zA-Z]+([\da-zA-Z]*[-._+])*[\da-zA-Z]+@[\da-zA-Z]+([-.][\da-zA-Z]+)*([\da-zA-Z]*[.])[a-zA-Z]{2,6}$/;
...
export const isWordSpace = (param: string): boolean => {
  ...
  const wordSpacePattern =
    /^[a-zA-Z\d,()'._&]+[\s\-a-zA-Z\d,()'._&]*[a-zA-Z\d,()'._?]*$/;
