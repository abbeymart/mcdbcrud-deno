...
export function computeUpdateQuery(
  tableName: string,
...
export function computeUpdateQueryById(
  tableName: string,
...
export function computeUpdateQueryByIds(
  tableName: string,
  actionParam: ActionParamType,
  ...
  try {
    // validate inputs
    if (
      tableName === "" || isEmptyObject(actionParam) || recordIds.length < 1
    ) {
...
export function computeUpdateQueryByParam(
  tableName: string,
  actionParam: ActionParamType,
  ...
  try {
    ...
    if (tableName === "" || isEmptyObject(actionParam) || !queryParams) {
      return errMessage(
        "tableName, queryParams (where-conditions) and actionParam are required.",
      );
    }
    ...
    const { whereQueryObject, ok, message } = computeWhereQuery(
      queryParams,
      ++fieldCount,
    );
    if (!ok) {
