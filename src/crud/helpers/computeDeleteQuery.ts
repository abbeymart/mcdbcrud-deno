...
export function computeDeleteQueryById(
  tableName: string,
  recordId: string,
): DeleteQueryResult {
  try {
    if (tableName === "" || recordId === "") {
      return errMessage(
        "tableName and recordId are required for the delete-by-id operation.",
      );
    }
...
export function computeDeleteQueryByIds(
  tableName: string,
  ...
  try {
    ...
    if (tableName === "" || recordIds.length < 1) {
      return errMessage(
        "tableName and recordIds are required for the delete-by-ids operation.",
      );
    }
...
export function computeDeleteQueryByParam(
  tableName: string,
  ...
  try {
    ...
    if (tableName === "" || isEmptyObject(queryParams)) {
      return errMessage(
        "tableName and queryParams (where-conditions) are required for the delete-by-param operation.",
      );
    }
