...
export function computeSelectQueryAll(
    ...
    try {
        ...
        Object.keys(modelRef).forEach((fieldName) =>
...
export function computeSelectQueryById(
    modelRef: ActionParamType,
    tableName: string,
    recordId: string,
    options: CrudOptionsType = {},
    ...
    try {
        // validate inputs
        if (
            tableName === "" || Object.keys(modelRef).length < 1 || recordId === ""
        ) {
...
export function computeSelectQueryByIds(
    modelRef: ActionParamType,
    tableName: string,
    ...
    try {
        // validate inputs
        if (
            tableName === "" || Object.keys(modelRef).length < 1 ||
            recordIds.length < 1
        ) {
        ...
        let selectQuery =
            `SELECT ${fieldText} FROM ${tableName} WHERE id IN ${inValues}`;
...
export function computeSelectQueryByParams(
    modelRef: ActionParamType,
    tableName: string,
    ...
    try {
        ...
        if (
            ...
            return errMessage(
                "tableName, queryParams (where-conditions) and modelRef are required.",
            );
        }
        // compute field-names
        const fieldNames: Array<string> = [];
