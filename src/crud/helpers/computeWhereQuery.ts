...
export function computeWhereQuery(
  ...
  try {
    ...
    if (isEmptyObject(queryParams) || !fieldPosition) {
      return errMessage(
        "queryParams (where-conditions) and fieldPosition (starting position for the where-condition-placeholder-values) are required.",
      );
    }
        ...
        case "object":
          ...
          if (Array.isArray(fieldValue) && fieldValue.length > 0) {
            ...
            if (fieldValue.every((val) => typeof val === "string")) {
              ...
              fieldValue.every((val) => typeof val === "number") ||
              ...
              return errMessage(
                `Where-query: Unsupported field-value-type for field-name: ${fieldName} and field-value: ${fieldValue}`,
              );
            }
            inValues += ")";
