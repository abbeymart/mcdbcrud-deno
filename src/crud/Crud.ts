...
export class Crud {
  ...
  constructor(params: CrudParamsType, options?: CrudOptionsType) {
    ...
    this.actionParams = params && params.actionParams
      ? params.actionParams
      : [];
    this.queryParams = params && params.queryParams ? params.queryParams : {};
    this.projectParams = params && params.projectParams
    ...
    this.limit = params.limit
      ...
      ? options.limit
      : 10000;
    this.checkAccess = options && options.checkAccess
      ? options.checkAccess
      : false;
    this.auditTable = options && options.auditTable
      ? options.auditTable
      : "audits";
    this.serviceTable = options && options.serviceTable
      ? options.serviceTable
      : "services";
    this.accessTable = options && options.accessTable
      ? options.accessTable
      : "accesses";
    ...
    this.serviceDb = options && options.serviceDb
      ? options.serviceDb
      : this.appDb;
    this.maxQueryLimit = options && options.maxQueryLimit
    ...
    this.cacheExpire = options && options.cacheExpire
      ? options.cacheExpire
      : 300;
    this.fieldSeparator = options?.fieldSeparator || "_";
    this.queryFieldType = options?.queryFieldType ||
      CrudQueryFieldType.Underscore;
    // unique cache-key
    ...
    this.usernameExistsMessage = options?.usernameExistsMessage
      ? options.usernameExistsMessage
      : "Username already exists. ";
    this.emailExistsMessage = options?.emailExistsMessage
      ? options.emailExistsMessage
  ...
  async recordsCount(): Promise<RecordCountResultType> {
    try {
      ...
      const countRowsRes = await this.appDb.queryObject(
        countQuery,
  ...
  async ownerRecordsCount(): Promise<OwnerRecordCountResultType> {
    try {
      ...
      // count owner-records
      const ownerScript =
        `SELECT COUNT(*) AS ownerrows FROM ${this.table} WHERE created_by = $1`;
      const ownerRowsRes = await this.appDb.queryObject(ownerScript, [
      ...
      switch (by.toLowerCase()) {
        case "id":
          ...
          if (this.recordIds.length === 1) {
            selectQueryResult = computeSelectQueryById(
              this.modelRef,
              this.table,
              this.recordIds[0],
              {
            ...
            selectQueryResult = computeSelectQueryByIds(
              this.modelRef,
              this.table,
              this.recordIds,
              {
        ...
        case "queryparams":
          ...
          selectQueryResult = computeSelectQueryByParams(
            this.modelRef,
            this.table,
            this.queryParams,
            {
  ...
  async getRoleServices(
    roleIds: Array<string>,
    serviceIds: Array<string>,
  ): Promise<Array<RoleServiceResponseType>> {
    ...
    try {
      ...
      const queryText =
        `SELECT service_id, role_id, service_category, can_read, can_create, can_update, can_delete, can_crud FROM ${this.roleTable}`;
      // roleIds-value
      ...
      // where-query
      const whereQuery =
        ` WHERE role_id IN ${recIds} AND service_id IN ${recIds2} AND is_active = $1`;
      const values = [true];
      const res = await this.accessDb.queryObject(
        queryText + whereQuery,
        values,
      ) as QueryObjectResult;
    ...
    try {
      ...
      if (userRec.userId && userRec.isActive) {
        if (idLen > 0) {
          ...
          const queryText =
            `SELECT COUNT(*) AS recordscount FROM ${this.table} WHERE created_by=$1`;
          const values = [userRec.userId];
          const res = await this.accessDb.queryObject(
            queryText,
            values,
          ...
          if (
            Number(res.rows[0].recordscount) > 0 &&
            this.taskType === TaskTypes.READ
          ) {
            ownerPermitted = true;
          }
        }
      ...
      if (
        ...
        !excludedTables.includes(this.table)
      ) {
        ownerPermitted = true;
      }
      ...
      const whereQuery =
        ` WHERE category IN ('table', 'Table', 'collection', 'Collection') AND name = $1`;
      ...
      if (
        ...
        ((res.rows[0].category as string).toLowerCase() === "table" ||
          "collection")
      ) {
  ...
  async taskPermissionById(
    ...
    try {
      ...
      if (this.recordIds.length < 1) {
        return getResMessage("unAuthorized", {
          message: "Document Ids not specified.",
        });
      }
      recordIds = this.recordIds;
      ...
      const recordFunc = (it1: string, roleFunc: RoleFuncType): boolean => {
        return roleRecords.some((it2: RoleServiceResponseType) =>
          roleFunc(it1, it2)
        );
      };
      ...
      switch (taskType) {
        ...
        case TaskTypes.UPDATE:
          ...
          recPermitted = recordIds.every((it1) =>
            recordFunc(it1, updateRoleFunc)
          );
          break;
        ...
        case TaskTypes.READ:
          ...
          // docs/records level access: every recordIds must have at least a match in the roleRecords
          recPermitted = recordIds.every((it1) =>
            recordFunc(it1, readRoleFunc)
          );
          break;
        default:
          return getResMessage("unAuthorized", {
            message: `Unknown/unsupported task-type (${taskType}`,
          });
      }

      // overall access permission
      taskPermitted = recPermitted || tablePermitted || ownerPermitted ||
        isAdmin;
      const ok: OkResponse = { ok: taskPermitted };
      const value = {
        ...ok,
        ...{ isAdmin, isActive, userId, roleId, roleIds },
      };
      if (taskPermitted) {
        return getResMessage("success", {
          value: value,
    ...
    try {
      ...
      if (this.currentRecs.length < 1) {
        ...
        if (currentRecRes.code !== "success") {
          return getResMessage("notFound", {
            message: "missing records, required to process permission",
          });
        }
        this.currentRecs =
          (currentRecRes.value as ObjectType)["records"] as Array<ObjectType>;
      }
  ...
  async checkLoginStatus(): Promise<ResponseMessage> {
    try {
      ...
      const accessRes = await this.accessDb.queryObject(
        queryText + whereQuery,
        values,
      ) as QueryObjectResult;
      ...
      if (accessRes.rows.length > 0) {
        ...
        return getResMessage("notFound", {
          message:
            `Access information for ${this.userInfo.loginName} not found. Login first, or contact system administrator`,
        });
      }

      // check if user exists
      const queryText2 =
        `SELECT id, role_ids, is_active, is_admin, profile FROM ${this.userTable}`;
      const whereQuery2 =
        ` WHERE id=$1 AND is_active=$2 AND (email=$3 OR username=$4)`;
      const values2 = [
        this.userInfo.userId,
        true,
        this.userInfo.loginName,
      ...
      const userRes = await this.accessDb.queryObject(
        queryText2 + whereQuery2,
        values2,
      ) as QueryObjectResult;
      ...
      if (userRes.rows.length < 1) {
        return getResMessage("notFound", {
          message:
            `User-profile information not found or inactive for ${this.userInfo.loginName}. Register a new account or contact system administrator. `,
        });
      }

      const checkAccessValue: CheckAccessType = {
        userId: userRes.rows[0].id as string,
        roleId: (userRes.rows[0].profile as unknown as ObjectType)
          .role_id as string,
